<script lang="ts">
  ...
  export let allowDrop: (item: Item, sourceDropZone: number) => boolean = () =>
    true;
  ...
  const startDragOff = () => {
    ...
    previouslyDraggedOver = [...previouslyDraggedOver, currentlyDraggingOver];
  ...
  $: {
    if (previouslyDraggedOver.length > 0 && !!hoverLeaveElementTweens) {
      ...
      previouslyDraggedOver = previouslyDraggedOver.map((target, index) => {
  ...
  $: {
    if ($dragTarget?.controllingDropZoneId === id) {
      ...
      if ($dragging === "dragging") {
        ...
        const hasDropTarget =
          overlapping.overlap.overlapX > 0 && overlapping.overlap.overlapY > 0;
        ...
        if (valid) {
          if (
            !!currentDropTarget &&
            currentDropTarget.dropTarget.id !== overlapping.target.id
          ) {
